...
public class Camera.MainWindow : Gtk.ApplicationWindow {
    ...
    construct {
        ...
        camera_revealer = new Gtk.Revealer ();
        camera_revealer.expand = true;
        camera_revealer.transition_type = Gtk.RevealerTransitionType.CROSSFADE;
        camera_revealer.add (clutter_embed);
        camera_revealer.reveal_child = true;

        stack.add_named (loading_view, "loading");
        stack.add_named (no_device_view, "no-device");
        stack.add_named (camera_revealer, "camera");
    ...
    private void initialize_camera_view (string camera_name) {
        ...
        camera_view.photo_saved.connect (() => {
            camera_revealer.reveal_child = false;
